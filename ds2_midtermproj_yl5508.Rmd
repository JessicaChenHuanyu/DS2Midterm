...
covid_factor_nonresp =
  covid |>
  select(where(is.factor))

chi_sq_matrix = matrix(NA, ncol = ncol(covid_factor_nonresp), nrow = ncol(covid_factor_nonresp))
for (i in 1:(ncol(covid_factor_nonresp)-1)) {
  for (j in (i+1):ncol(covid_factor_nonresp)) {
    cross_table = table(covid_factor_nonresp[,i, drop = TRUE],
                        covid_factor_nonresp[,j, drop = TRUE])
    chi_sq_matrix[i,j] = chisq.test(cross_table)$p.value
  }
}

rownames(chi_sq_matrix) = colnames(covid_factor_nonresp)
colnames(chi_sq_matrix) = colnames(covid_factor_nonresp)
...
ggplot(covid_factor_long, aes(x = value, y = recovery_time)) +
  geom_violin() +
  facet_wrap(~predictor, scales = "free")

anova_matrix = matrix(NA, ncol = ncol(covid_numeric), nrow = ncol(covid_numeric))
for (i in 1:(ncol(covid_numeric))) {
  for (j in 1:ncol(covid_numeric)) {
    cross_dat = data.frame(num_1 = covid_numeric[,i, drop = TRUE], 
                           num_2 = covid_numeric[,j, drop = TRUE])
    anova_matrix[i,j] = summary(aov(num_1 ~ num_2, data = cross_dat))[[1]]$"Pr(>F)"[[1]]
  }
}

rownames(anova_matrix) = colnames(covid_numeric)
colnames(anova_matrix) = colnames(covid_numeric)
...
# test error (rmse)
rf.test = predict(rf.fit, x = xtest)
rf.rmse = sqrt(mean((rf.test - ytest)^2))
rf.rmse

ctree.test = predict(ctree.fit, x = xtest)
ctree.rmse = sqrt(mean((ctree.test - ytest)^2))
ctree.rmse
```
