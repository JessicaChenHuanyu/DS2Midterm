...
```{r}
set.seed(11)
pls = plsr(recovery_time ~ ., data = training, 
...
# test error (MSE)
pred.pls = predict(pls, newdata = testing,
ncomp = ncomp.cv)
sqrt(mean((testing$recovery_time - pred.pls)^2))
```
...
```{r message = FALSE, warning = FALSE}
set.seed(11)
mars_grid = expand.grid(degree = 1:5, nprune = 2:25)
ctrl = trainControl(method = 'cv', number = 10)
...
p1 = pdp::partial(mars.fit, pred.var = c("height"), grid.resolution = 10) %>% autoplot()

p2 = pdp::partial(mars.fit, pred.var = c("height", "weight"),
grid.resolution = 10) %>%
...
mars.rmse = mean((mars.test - ytest)^2)
mars.rmse
```
...
```{r}
bwplot(resamples(list(lasso = lasso.fit, lasso_1se = lasso.fit, enet = enet.fit, enet_1se = enet.fit.1se, mars = mars.fit, gam = gam.fit)), metric = "RMSE")
```
